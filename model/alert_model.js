...
class Alert{
    ...
    static getAllAlerts(limit=0, start = 0)
    {
        ...
        let sql = `SELECT alertId,masterIdType as alertName,title,CONCAT('${BASE_URL}','',image) as alertImage, 
                   (SELECT COUNT(alertIdFK) FROM alerttracking WHERE isRead = 0 AND alertId = alertIdFK)as isRead, 
    ...
    static getAlertTrackCount(alertId)
    {
        let sql = `SELECT COUNT(alertTrackingId) as totalAlertTrack,
                   (SELECT COUNT(alertIdFK) FROM alerttracking WHERE isRead = 0 AND alertIdFK = ${alertId})as isRead,
