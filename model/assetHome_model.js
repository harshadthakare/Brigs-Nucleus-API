class AssetHome {
    ...
    static getRootCategory(organizationIdFK) {
        let sql = `SELECT categoryId,title, (SELECT COUNT(a.assetId) AS totalAssets FROM asset a JOIN assetcatrelation a1 ON a1.assetIdFK = a.assetId WHERE a.organizationIdFK = ${organizationIdFK} AND a1.categoryIdFK = categoryId AND a.isDeleted = 0) as totalAssets, 
        ...
        (SELECT COUNT(d.documentId) AS totalDocuments FROM document d JOIN documenttype dt ON d.documentTypeIdFK = dt.documentTypeId WHERE d.documentTypeIdFK = 1 AND d.masterId = categoryId AND d.isDeleted = 0) AS totalDocuments,
