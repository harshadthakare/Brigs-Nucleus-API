...
import Dept from "../model/department";
...
/**
 * @swagger
 * /departments/departmentList/{parentId}:
 *   get:
 *     tags:
 *       - Department
 ...
 *     parameters:
 *       - name: parentId
 *         description: "parent id always starts with 0"
 *         in: path
 *         required: true
 *       - name: Authorization
 *         description: token
...
router.get("/departmentList/:parentId", (req, res, next) => {
    verifyToken(req, res, organizationIdFK => {

        let parentId = req.params.parentId;

        db.query(Dept.getAllDeptSQL(parentId, organizationIdFK), (err, data) => {
            if (!err) {
