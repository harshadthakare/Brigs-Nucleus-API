...
router.get("/listOfUsers/:pageNo", (req, res, next) => {
    verifyToken(req, res, adminId => {
        const limit = 10;
        const page = req.params.pageNo;
        let deptId = req.body.departmentIdFK;
        var pageCount1 = 0;

        db.query(User.getAllUserSQL(deptId), (err1, data1) => {
            if (data1) {
                pageCount1 = data1.length;

                db.query(User.getAllUserSQL(deptId, limit, page), (err, data) => {
                    if (!err) {
