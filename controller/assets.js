...
import { generateQrCode } from "../config/generateQrCode"
...
router.get("/AssetSearch/", (req, res, next) => {
    verifyToken(req, res, organizationIdFK => {

        let categoryId = req.query.categoryIdFK;
...
router.post("/addAsset", [
    ...
    verifyToken(req, res, organizationIdFK => {
        ...
        db.query(asset.addAssetSQL(), (err, data) => {
            ...
            if (!err) {
                generateQrCode(assetCode, result => {
                    if (result) {
